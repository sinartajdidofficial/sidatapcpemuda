...
import { Plus, Trash2, Edit, Users, Loader2, ArrowLeft } from 'lucide-react';
...
const emptyForm: Form = { nama: '', tempat_lahir: '', tanggal_lahir: '', alamat: '', pendidikan_terakhir: '', no_whatsapp: '' };
...
export default function AnggotaPage() {
  ...
  const saveMutation = useMutation({
    mutationFn: async () => {
      if (editId) {
        const { error } = await supabase.from('anggota').update(form).eq('id', editId);
        if (error) throw error;
      } else {
  ...
  function openEdit(item: typeof list[0]) {
    setEditId(item.id);
    setForm({ nama: item.nama, tempat_lahir: item.tempat_lahir, tanggal_lahir: item.tanggal_lahir, alamat: item.alamat, pendidikan_terakhir: item.pendidikan_terakhir, no_whatsapp: item.no_whatsapp });
    setOpen(true);
  }
